...
class spo2(object):
    ...
    def __init__(self, bus):
        super(spo2, self).__init__()
        self.bus = bus      # Bus for smbus --> use 1 unless something crazy is going on
        
    def request_reading(self,data_add):
        reading=hex(bus.read_byte_data(slave_add,data_add))
        return reading

    def write_data(self,data_add,data_byte):
        bus.write_byte_data(slave_add,data_add,data_byte)

    def mode_enable(self,mode_byte):
        write_data(Mode_Configuration,mode_byte)
        if mode_byte != Reset:
            print("Mode set successfully")

    def SPO2_config(self,SPO2_SR,LED_PW):
        byte=(0b01000000)|(SPO2_SR<<2)
        byte=byte|LED_PW
        write_data(SPO2_Configuration,byte)
        print("SPO2 configuration successful")

    def LED_config(self,RED_PA,IR_PA):
        byte=(RED_PA<<4)|(IR_PA)
